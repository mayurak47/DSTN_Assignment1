...
void l1_valid_TagPrint ()
{
	for (int i=0; i<ENTRIES; i++)
	{
		printf(" : %d", l1_data->L1row[i].valid_Tag[0]);
		printf(" : %d", l1_data->L1row[i].valid_Tag[1]);
		printf(" : %d", l1_data->L1row[i].valid_Tag[2]);
		printf(" : %d\n", l1_data->L1row[i].valid_Tag[3]);
	}
}
...
void l1_checkInitialization()
{
	for (int i=0; i<ENTRIES; i++)
	{
		printf("%d\n", l1_or_array((char *)&(l1_data->L1row[i]), sizeof(l1_data->L1row[i])));
	}
}
...
int l1_LRU_victimBlock(int index)
{
	short lruBits = l1_data->L1row[index].LRUBits;
...
void l1_printLRUBits(int index)
{
	l1_BinaryRepShort(l1_data->L1row[index].LRUBits);
}
...
void l1_LRU_Update(int index, char wayPrediction)  //on hit
{
	...
	{
		case 0:
			l1_data->L1row[index].LRUBits |= 0b1111000000000000;
			l1_data->L1row[index].LRUBits &= 0b0111011101110111;
			break;
		case 1:
			l1_data->L1row[index].LRUBits |= 0b0000111100000000;
			l1_data->L1row[index].LRUBits &= 0b1011101110111011;
			break;
		case 2:
			l1_data->L1row[index].LRUBits |= 0b0000000011110000;
			l1_data->L1row[index].LRUBits &= 0b1101110111011101;
			break;
		case 3:
			l1_data->L1row[index].LRUBits |= 0b0000000000001111;
			l1_data->L1row[index].LRUBits &= 0b1110111011101110;
			break;
	}
	if (DEBUG) 
	{
...
void l1_write_to_l1(int physical_address, char data){
	...
	int way_no = l1_data->L1row[index].wayPrediction;

	l1_data->L1row[index].data[L1_BLOCK*way_no + offset] = data;

	for (int i =0; i<L1_BLOCK; i++)
	{
		bus16B[i] = l1_data->L1row[index].data[L1_BLOCK*way_no + i];
	}
...
void l1_cacheMiss(int TLBaddress, int index)
{
	...
	l1_data->L1row[index].valid_Tag[victimBlock] = framebits;
	for (int i =0; i<L1_BLOCK; i++)
	{
		l1_data->L1row[index].data[L1_BLOCK*victimBlock + i] = bus16B[i];
	}
	l1_data->L1row[index].wayPrediction = victimBlock;
}
...
int l1_comparator(int TLBaddress, char wayPrediction, int index)
{
	...
	{
		case 0:
			valid_Tag = l1_data->L1row[index].valid_Tag[0];
			break;
		case 1:
			valid_Tag = l1_data->L1row[index].valid_Tag[1];
			break;
		case 2:
			valid_Tag = l1_data->L1row[index].valid_Tag[2];
			break;
		case 3:
			valid_Tag = l1_data->L1row[index].valid_Tag[3];
			break;
		default:
...
char l1_search_cache(int TLBaddress, int index, int offset)
{
	...
	return_comparator = l1_comparator(TLBaddress, l1_data->L1row[index].wayPrediction, index);
	...
	{
		// cache hit
		if (DEBUG) printf("Cache Hit on way Prediction : %d\n", l1_data->L1row[index].wayPrediction);
		if (LOG) printf("Cache Hit\n");
		return  l1_data->L1row[index].data[L1_BLOCK*return_comparator + offset];
	}
	if (DEBUG) printf("Cache Miss on Way Prediction : %d\n",l1_data->L1row[index].wayPrediction);

	// checks done in parallel in hardware
	return_comparator = l1_comparator(TLBaddress, (l1_data->L1row[index].wayPrediction+1)%4, index);
	if (return_comparator >= 0)
	{
		// cache hit
		if (DEBUG) printf("Cache Hit after Way Prediction : %d\n", (l1_data->L1row[index].wayPrediction+1)%4);
		if (LOG) printf("Cache Hit\n");
		return  l1_data->L1row[index].data[L1_BLOCK*return_comparator + offset];
	}
	return_comparator = l1_comparator(TLBaddress, (l1_data->L1row[index].wayPrediction+2)%4, index);
	if (return_comparator >= 0)
	{
		// cache hit
		if (DEBUG) printf("Cache Hit after Way Prediction : %d\n", (l1_data->L1row[index].wayPrediction+2)%4);
		if (LOG) printf("Cache Hit\n");
		return  l1_data->L1row[index].data[L1_BLOCK*return_comparator + offset];
	}
	return_comparator = l1_comparator(TLBaddress, (l1_data->L1row[index].wayPrediction+3)%4, index);
	if (return_comparator >= 0)
	{
		// cache hit
		if (DEBUG) printf("Cache Hit after Way Prediction : %d\n", (l1_data->L1row[index].wayPrediction+3)%4);
		if (LOG) printf("Cache Hit\n");
		return  l1_data->L1row[index].data[L1_BLOCK*return_comparator + offset];
	}
